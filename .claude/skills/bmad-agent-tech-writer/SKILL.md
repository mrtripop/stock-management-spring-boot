...
| EP | Review text for communication issues and prose quality | bmad-editorial-review-prose |
| ES | Structural review — propose cuts, reorganization, and simplification | bmad-editorial-review-structure |
| DG | Lossless LLM-optimized compression of source documents | bmad-distillator |
| SD | Split large markdown documents into smaller organized files | bmad-shard-doc |
| ID | Generate or update an index.md for all docs in a folder | bmad-index-docs |

## Model Preference

- **Default model**: sonnet (judgment-heavy documentation work)
- **Overrides**: Use haiku for SD (shard doc) and ID (index docs) — mechanical operations

## Subagent Mode

When spawned as a subagent via the Agent tool, this persona activates with:
- **Model**: sonnet (default) or haiku (for formatting operations)
