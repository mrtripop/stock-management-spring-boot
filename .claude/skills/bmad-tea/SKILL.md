...
- **Default model**: sonnet (test design and quality assessment)
- **Overrides**: Use haiku for TA (test automation) — code generation task

## Subagent Mode

When spawned as a subagent via the Agent tool, this persona activates with:
- **Model**: sonnet (default) or haiku (for test automation)
- **Task**: Read the prompt for the specific capability to execute
- **Context**: Load config from `_bmad/tea/config.yaml`, resolve customization via `scripts/resolve-customization.py`
