...
export function SizeScale() {
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[var(--color-text-primary)]">Typography Size Scale</h2>
      <p className="text-sm text-[var(--color-text-secondary)]">
        Font family: <code className="px-1.5 py-0.5 bg-[var(--color-surface)] rounded text-xs font-mono">{theme.typography.fontFamily}</code>
      </p>
          ...
          return (
            <div key={key} className="flex items-baseline gap-4 p-3 rounded-[var(--radius-md)] border border-[var(--color-border)]">
              ...
              <div className="text-right shrink-0">
                <div className="text-[0.625rem] text-[var(--color-text-muted)]">{step.size}</div>
                <div className="text-[0.625rem] text-[var(--color-text-muted)]">lh {step.lineHeight}</div>
                <div className="text-[0.625rem] text-[var(--color-text-muted)]">w {step.weight}</div>
              </div>
...
export function FontWeightScale() {
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[var(--color-text-primary)]">Font Weights</h2>
      <div className="space-y-3">
        {WEIGHTS.map(({ key, token, value }) => (
          <div key={key} className="flex items-baseline gap-4 p-3 rounded-[var(--radius-md)] border border-[var(--color-border)]">
            <div className="w-24 text-right shrink-0">
              <span className="text-[0.625rem] font-mono text-[var(--color-text-muted)]">{token}</span>
            </div>
            <div className="flex-1 text-2xl" style={{ fontWeight: `var(${token})` }}>
              Pharmacy Stock Management
            </div>
            <div className="w-12 text-right shrink-0">
              <span className="text-[0.625rem] text-[var(--color-text-muted)]">{value}</span>
            </div>
          </div>
        ))}
      </div>
