...
**Files:**
- Rewrite: `demo-ui/src/pages/Login.jsx`

- [ ] **Step 1: Rewrite Login.jsx with teal theme**

Replace `demo-ui/src/pages/Login.jsx`:
  ...
  const handleMfa = async (e) => {
    ...
    try {
      ...
      if (payload?.token) {
        ...
        // tempToken was already set — might be the final token
        onLogin()
        navigate('/')
      }
    } catch (err) {
...
Expected: Teal gradient background, centered white card with "P" logo, Login/Register tabs.

- [ ] **Step 3: Commit**

```bash
git add demo-ui/src/pages/Login.jsx
git commit -m "feat(demo-ui): redesign Login page with teal pharmacy theme

Full-screen gradient background, centered card with logo.
Login/Register tabs with MFA verification step.
Uses atom components (Button, Input) and FormField molecule."
```
